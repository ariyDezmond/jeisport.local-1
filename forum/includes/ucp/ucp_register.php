...
class ucp_register
{
	...
	function main($id, $mode)
	{
        ...
        $fromExternal = $request->is_set_post('from_external');
		$coppa			= $request->is_set('coppa') ? (int) $request->variable('coppa', false) : false;
		$agreed			= $request->variable('agreed', $fromExternal ? true : false);
		...
		if ($submit)
		{
			...
			if (!check_form_key('ucp_register') && !$fromExternal)
			{
			...
			if ($config['enable_confirm'] && !$fromExternal)
			{
