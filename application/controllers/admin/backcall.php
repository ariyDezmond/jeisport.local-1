...
class Backcall extends CI_Controller {
    ...
    public function page($page = 'backcall', $msg = null) {
        if (!$this->session->userdata('logged')) {
            redirect('admin/login');
        }
        if (!$msg) {
            $data['msgs'] = $this->backcall_model->get_msgs();
            $data['title'] = 'Заявки на обратный звонок';
            $this->load->view('admin/templates/metahead', $data);
            $this->load->view('admin/templates/navbar', $data);
            $this->load->view('admin/pages/settings/templates/left-nav', $data);
            ...
            $data['msg'] = $this->backcall_model->get_msgs($msg);
            $data['title'] = 'Заявки на обратный звонок';
            $this->load->view('admin/templates/metahead', $data);
            $this->load->view('admin/templates/navbar', $data);
            $this->load->view('admin/pages/settings/templates/left-nav', $data);
    ...
    public function setread() {
        ...
        if ($this->input->post('id')) {
            ...
            $this->backcall_model->update_backcall_read($id);
        }
    }
    
    public function delete($id) {
        $this->backcall_model->backcall_delete($id);
        redirect('admin/settings/backcall');
    }
