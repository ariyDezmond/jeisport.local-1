...
class Blog extends CI_Controller {
    ...
    public function page($page = 'blog') {
        if (!$this->session->userdata('logged')) {
            redirect('admin/login');
        }
        ...
        if ($this->input->post('do') == 'blogEdit') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($_FILES['image']['name'] == '') {
                    ...
                    $this->blogs_model->update_blog($id);
                    $arr = array(
                    ...
                    if (!$this->upload->do_upload('image')) {
                        ...
                        if (file_exists('images/blog/' . $blog['image'])) {
                            ...
                            $image_data = $this->upload->data();
                            $this->blogs_model->update_blog($id, $image_data['file_name']);
                            $arr = array(
                                'error' => '<div class="alert alert-success" role="alert"><strong>Успех! </strong>Пост был успешно обновлен!</div>'
                            );
                            $this->session->set_userdata($arr);
                            ...
                            if ($this->input->post('tags')) {
                                foreach ($this->input->post('tags') as $tag) {
                                    $this->blogs_model->set_tag($tag, $id, $object);
                                }
                            }
                            $image_data = $this->upload->data();
                            $this->blogs_model->update_blog($id, $image_data['file_name']);
    ...
    public function check_url($url) {
        if ($this->blogs_model->get_blog_by_url($url)) {
        ...
        if ($this->input->post('do') == 'blogAdd') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!$this->upload->do_upload('image')) {
                    ...
                    $image_data = $this->upload->data();
                    $this->blogs_model->set_blog($image_data['file_name']);
    ...
    public function up($id) {
        $this->blogs_model->order_up($id);
    }

    public function down($id) {
        $this->blogs_model->order_down($id);
    }
